...
import {
    ...
    group3,
    hireStaff,
    repairHouse, selectionSort, sortFilteringArray, splitSameLetter
} from "./03";
...
test("sort function using selection", () => {
        const arr = [5, 3, 6, 2, 10]
        const result = selectionSort(arr)

        expect(result).toStrictEqual([2, 3, 5, 6, 10]   )
    }
)

test("sort function use selection", () => {
        const arr = [5, 3, 6, 2, 10]
        const result = sortFilteringArray(arr)
